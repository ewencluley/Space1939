...
namespace Space1939
{
    class Record:DrawableGameComponent, ICollidable
    {
        Game1 game;
        Model theModel;
        float scale;
            ...
            if (!collected)
            {
                ...
                foreach (ModelMesh mesh in theModel.Meshes)
                {
                    foreach (BasicEffect effect in mesh.Effects)
                    {
                        ...
                        effect.World = mesh.ParentBone.Transform * Matrix.CreateScale(scale, scale, scale) * Matrix.CreateRotationY(rotation) * Matrix.CreateTranslation(position);
